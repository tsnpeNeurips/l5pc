...
def sample_and_simulate(cfg: DictConfig) -> None:
    ...
    log.info(f"Saved seed! {time.time() - start_time}")

    prior = assemble_prior(cfg)
    sim_and_stats = assemble_simulator(cfg.model)
    theta_db, x_db = assemble_db(cfg)

    np.savetxt(f"seed2.txt", [seed], fmt="%d")
    log.info(f"Saved seed middle {time.time() - start_time}")
    ...
    np.savetxt(f"seed3.txt", [seed], fmt="%d")
    log.info(f"Saved seed at end! {time.time() - start_time}")
