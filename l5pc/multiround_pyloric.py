...
from torch.distributions import biject_to, TransformedDistribution
...
def simulate_mp(cfg, round_):
    ...
    prior_tf = TransformedDistribution(prior, bijection.inv)
...
def train(cfg, round_):
    ...
    bijection = biject_to(prior.support)
    prior_tf = TransformedDistribution(prior, bijection.inv)
    prior_tf, _, _ = process_prior(prior_tf)

    if round_ == 1:
        path = "/path/path/path/path/tsnpe_collection/l5pc/results/p31_4/prior_predictives_energy_paper"
    ...
    theta = bijection.inv(as_tensor(np.asarray(theta), dtype=float32))
    ...
    if round_ > 1 or cfg.retrain_first_round:
        if cfg.parallel_training:
            ...
            for seed in range(cfg.ensemble_size):
                ...
                if round_ > 1:
                    ...
                    inference = SNPE(prior=prior_tf, density_estimator=dens_estim)
        ...
        pa = "/path/path/path/path/tsnpe_collection/l5pc/results/p31_2/multiround"
...
def evaluate(cfg, round_):
    ...
    bijection = biject_to(prior.numerical_prior.support)
    theta_torch = posterior.sample((cfg.num_predictives,))
    log.info(f"theta_torch before {theta_torch}")
    theta_torch = bijection(theta_torch)
    log.info(f"theta_torch after {theta_torch}")
    ...
    with mpl.rc_context(
        fname="/path/path/path/path/tsnpe_collection/l5pc/.matplotlibrc"
    ):
        ...
        posterior_samples = bijection(posterior_samples)
    ...
    # if round_ == 1:
    #     path = "/path/path/path/path/tsnpe_collection/l5pc/results/p31_2/prior_predictives_energy_paper"
    #     theta = pd.read_pickle(join(path, "all_circuit_parameters.pkl"))
    #     x = pd.read_pickle(join(path, "all_simulation_outputs.pkl"))
...
#     with open(
#         "/path/path/path/path/tsnpe_collection/l5pc/results/pyloric_restricted_prior.pkl",
#         "rb",
#     ) as handle:
...
def load_pyloric_posterior_from_file(path, round_):
    ...
    base = "/path/path/path/path/tsnpe_collection/l5pc/results/p31_4/multiround"
